...
ARG BASE_IMAGE=amitie10g/kali
ARG BASE_IMAGE_VARIANT=rolling
FROM $BASE_IMAGE-$BASE_IMAGE_VARIANT:upstream
...
RUN adduser --quiet --add_extra_groups --disabled-password --gecos "" kali && \
    ...
    apt-get update && \
    apt-get install --no-install-suggests -y nano lynx tor make iptables && \
    cd /tmp/kalitorify && \
    make install
...
RUN apt-get install -y kali-desktop-xfce xrdp kali-tools-top10 chromium
