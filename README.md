...
* ``upstream`` the base image built with the [Phusion's base image](https://github.com/phusion/baseimage-docker) project modifications<sup>[2]</sup>
...
* ``latest`` ``desktop`` base desktop (XFCE and XRDP) plus Kali top 10 tools and Chromium browser (small)<sup>[3]</sup>
  * ``kali-tools-top10``
  * ``kali-desktop-xfce``
  * ``xrdp``
  * ``chromium``
...
```
docker build --build-arg APT_PROXY=$(docker inspect -f '{{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}}' apt-cacher-ng) -t amitie10g/kali:<tag> .
```
